...
//        }
        if ("".equals(mDiscriptionEdit.getText().toString())) {
          Toast.makeText(PublishActivity.this, "请输入详细评论", Toast.LENGTH_SHORT).show();
          return;
        }
...
//        }
        ...
        AVObject pingLun = new AVObject("PingLun");
//        pingLun.put("title", mTitleEdit.getText().toString());
        pingLun.put("description", mDiscriptionEdit.getText().toString());
//        pingLun.put("price", Integer.parseInt(mPriceEdit.getText().toString()));
        pingLun.put("owner", AVUser.getCurrentUser());
        pingLun.put("image", new AVFile("pingLunPic", mImageBytes));
        pingLun.saveInBackground(new SaveCallback() {
          @Override
